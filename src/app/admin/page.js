...
const AdminPage = () => {
  ...
  // New State for Pagination and Date Filters
  ...
  const fetchWorkshops = async (page = 1, filters = dateFilters) => {
    ...
    if (data) {
      setAllSessions(data);
      setFilteredSessions(data); // The API now does the filtering
  ...
  }, [dateFilters]); // Refetch when date filters change
  
  // We need to fetch total stats separately now or they'll be filtered
  const fetchTotalStats = async () => {
    const response = await fetch('/api/get-workshops'); // Fetch all for stats
      ...
      <div className="mt-12">
        <div className="space-y-4 md:space-y-0 md:flex md:justify-between md:items-center">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:space-x-4">
            <input type="date" name="startDate" value={dateFilters.startDate} onChange={handleDateFilterChange} className="px-4 py-2 border border-gray-300 rounded-md"/>
            <input type="date" name="endDate" value={dateFilters.endDate} onChange={handleDateFilterChange} className="px-4 py-2 border border-gray-300 rounded-md mt-2 md:mt-0"/>
            <input 
              type="text" 
              placeholder="Search by title or artist..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="px-4 py-2 border border-gray-300 rounded-md mt-2 md:mt-0"
            />
          </div>
        </div>
